...
function popUpRandomMole() {
  ...
  const moleHeads = document.querySelectorAll('.garden_mole');
  ...
  moleHead.classList.remove('garden_mole_hidden', 'wgs__mole-head--whacked');
...
function hideMole(mole) {
  clickable = false;
  mole.classList.add('garden_mole_hidden');
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  for (let moleHead of moleHeads) {
    moleHead.addEventListener('click', event => {
      ...
      // UNCOMMENT THIS LINE OF CODE WHEN DIRECTED
      event.target.classList.add('garden_mole_hidden');
